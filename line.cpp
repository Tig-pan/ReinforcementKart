...
float Line::angleToLine(float px, float py, float vx, float vy, Line* line)
{
	float lineWX = line->x1 - line->x2;
	float lineWY = line->y1 - line->y2;
	...
	float vectorToClosestX = closestX - px;
	float vectorToClosestY = closestY - py;

	dot = vectorToClosestX * vx + vectorToClosestY * vy;
	float closestVectorMagnitude = sqrtf(vectorToClosestX * vectorToClosestX + vectorToClosestY * vectorToClosestY);
	float velocityVectorMagnitude = sqrtf(vx * vx + vy * vy);

	return acosf(dot / (closestVectorMagnitude * velocityVectorMagnitude));
}
...
bool Line::getLineSegmentIntersection(Line* kLine, Line* wLine, float* ox, float* oy, float* onx, float* ony, float* otime)
{
	...
	float kFraction = (lineWX * (kLine->y1 - wLine->y1) - lineWY * (kLine->x1 - wLine->x1)) / denominator; // aka time
	float wFraction = (lineKX * (kLine->y1 - wLine->y1) - lineKY * (kLine->x1 - wLine->x1)) / denominator;
